...
/*目的：从格式为“时间，msg，fromCtiy，toCtity，method，OTA”统计出数据格式为“时间，OTA次数”*/
public class OtaTotal implements Processor<String, String> {
    ...
    public void process(String key, String value) {
        ...
        for (String data : valueLog
            ...
            if (data.equals("\n")) {
                ...
                String ota = store.get(4);
                newOne.add(time);
                newOne.add(ota);
                newOne.add("1");
                String newOneString = StringUtils.join(newOne, ",");
                /*如果原来的航线存在则存入新航线且次数为一*/
                String judge = this.kvStore.putIfAbsent(ota, newOneString);
                newOne.clear();
                /*如果原来的航线存在则次数自加一*/
                if (judge != null) {
                    try (KeyValueIterator<String, String> iter = kvStore.all()) {
                        while (iter.hasNext()) {
                            ...
                            if (getKey.equals(ota)) {
                                ...
                                this.kvStore.put(ota, addOneString);
                                break;
                            }
                        }
                    }
